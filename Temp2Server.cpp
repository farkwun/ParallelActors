...
void AddToCurrentActors(std::string new_actor_id, Actor new_actor){
  std::lock_guard<std::mutex> lck(current_actors_mtx);
...
void AddNewActors(){
  ...
  while(it != temp.end()){
    ...
    AddToCurrentActors(new_actor_id, new_actor);
